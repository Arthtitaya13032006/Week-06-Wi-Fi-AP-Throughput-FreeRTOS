...
// --- แก้ไขเป็นชื่อและรหัสผ่าน Hotspot ของคุณ ---
#define AP_SSID            "Pornprom"
#define AP_PASS            "12345678"
// ------------------------------------------

// --- คุณจะต้องเปลี่ยน IP นี้ให้ตรงกับ IP ของเครื่องคอมพิวเตอร์ของคุณ (IPv4 Address) 
// หลังจากที่คอมพิวเตอร์เชื่อมต่อ Hotspot "Pornprom" แล้ว
#define SERVER_IP          "10.241.132.185" // <<<<< อย่าลืมเปลี่ยน IP ตรงนี้!
// ------------------------------------------
...
static void perform_throughput_test(int benchmark_round)
{
    ...
    // --- ย้ายการพิมพ์ค่า RSSI ขึ้นมาแสดงก่อนเชื่อมต่อ Socket ---
    ESP_LOGI(TAG, "=======================================================");
    ESP_LOGI(TAG, " [BENCHMARK ROUND %d/%d] Attempting to connect to Server...", benchmark_round, BENCHMARK_ROUNDS);
    ...
    if (connect(socket_fd, (struct sockaddr *)&destination, sizeof(destination)) != 0) {
        ESP_LOGW(TAG, "Socket connect failed to %s:%d. Please start the Python server on your computer.", SERVER_IP, SERVER_PORT);
        close(socket_fd);
        ESP_LOGI(TAG, "=======================================================\n");
        return;
    }
